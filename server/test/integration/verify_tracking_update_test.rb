...
class VerifyTrackingUpdateTest < ActionDispatch::IntegrationTest
  ...
  test 'new tracking update should increment count' do
    assert_difference('CurrentStatus.count') do
      put "/track/#{@new_wk_status[:workstation_name]}/#{@new_wk_status[:status]}/#{@new_wk_status[:os]}/#{@new_wk_status[:user_hash]}"
    end
  end

  test 'existing tracking update should not increment count' do
    assert_no_difference('CurrentStatus.count') do
      put "/track/#{@exiting_wk_status[:workstation_name]}/#{@exiting_wk_status[:status]}/#{@exiting_wk_status[:os]}/#{@exiting_wk_status[:user_hash]}"
    end
  end
  ...
  test 'most recently updated status should be displayed first' do
    ...
    assert response.content_type.to_s == CONTENT_TYPE_JSON, "Expected Content-Type: #{CONTENT_TYPE_JSON}, got: #{response.content_type.to_s}"
